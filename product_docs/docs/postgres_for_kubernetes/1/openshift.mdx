...
1.  Move to a `stable-vX.Y` channel to remain in a minor release (e.g. `stable-v1.22` would
    remain in the 1.22 minor LTS release, consuming future patch releases).
...
-   `@@REPOSITORY@@` is the name of the repository, as explained in ["Which repository to
    choose?"](private_edb_registries.md#which-repository-to-choose)
...
Again, refer to the [relevant OpenShift document page](https://docs.openshift.com/container-platform/4.16/observability/monitoring/enabling-monitoring-for-user-defined-projects.html)
for further detail.
